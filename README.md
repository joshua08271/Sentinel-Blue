...
The Windows restore path creates its replacement file with the complete approved security descriptor, then restores the security stream on the same exclusive native handle. It verifies the result and still refuses publication unless the entire descriptor matches. See [endurance acceptance](docs/ENDURANCE_ACCEPTANCE.md) for the remaining practice-event gates.
