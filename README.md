...
The Windows restore path also reapplies captured inheritance-protection flags when the backup API changes them, including protection on an absent SACL. It uses the held file handle and still refuses publication unless the complete approved security descriptor matches. See [endurance acceptance](docs/ENDURANCE_ACCEPTANCE.md) for the remaining practice-event gates.
