...
The Windows restore path creates its replacement file with the complete approved security descriptor, including protection on an absent SACL. It verifies the result on the exclusive native handle and still refuses publication unless the entire descriptor matches. See [endurance acceptance](docs/ENDURANCE_ACCEPTANCE.md) for the remaining practice-event gates.
