...
class SetupTests(unittest.TestCase):
    ...
    def test_host_deploys_while_other_host_setup_is_still_running(self):
        transport = FixtureTransport()
        deployed_first = threading.Event()
        original = transport.execute
        overlapped = []
        def execute(host, script, seconds):
            if host['name'] == 'host-1' and not script.startswith('# check'):
                overlapped.append(deployed_first.wait(2))
            return original(host, script, seconds)
        transport.execute = execute
        calls = []
        def complete(host, seconds):
            self.assertNotIn(host, transport.active)
            self.assertIn(host, transport.ready)
            calls.append(host)
            if host == 'host-0':
                deployed_first.set()
            return {'status':'completed'}
        plan = self.plan()
        report = SetupRunner(plan, self.profile, transport=transport, probe=healthy_probe,
                             on_host_ready=complete).execute(self.root/'state', plan_digest(plan))
        self.assertEqual(report['status'], 'ready')
        self.assertEqual(overlapped, [True])
        self.assertEqual(calls, ['host-0', 'host-1'])
        with self.assertRaisesRegex(ValueError, 'cannot be replayed'):
            SetupRunner(plan, self.profile, on_host_ready=complete).execute(
                self.root/'state', plan_digest(plan), resume=True)

    def test_host_deployment_failure_blocks_overall_readiness(self):
        transport = FixtureTransport()
        plan = self.plan()
        report = SetupRunner(plan, self.profile, transport=transport, probe=healthy_probe,
                             on_host_ready=lambda *_: {'status':'uncertain'}).execute(
                                 self.root/'state', plan_digest(plan))
        self.assertEqual(report['status'], 'incomplete')
        self.assertTrue(all(x['status'] == 'uncertain' for x in report['host_completions'].values()))
    ...
    def test_ready_host_starts_while_other_host_is_still_connecting(self):
        started = threading.Event()
        transport = FixtureTransport()
        original_preflight, original_execute = transport.preflight, transport.execute

        def preflight(host, seconds):
            if host["name"] == "host-0":
                # A barrier-based scheduler would time out here before host-1
                # could execute. The ready host must make real forward progress.
                return CommandResult(0 if started.wait(2) else 1, 0)
            return original_preflight(host, seconds)

        def execute(host, script, seconds):
            if host["name"] == "host-1" and script.startswith("# apply"):
                started.set()
            return original_execute(host, script, seconds)

        transport.preflight, transport.execute = preflight, execute
        report = self.execute(transport)
        self.assertEqual(report["status"], "ready")
        self.assertTrue(report["host_preflight"]["host-0"])
        self.assertTrue(report["under_3_minutes"])

    def test_final_checks_overlap_hosts_and_preserve_drift_failure(self):
        barrier = threading.Barrier(2)
        transport = FixtureTransport()
        transport.ready.update({"host-0", "host-1"})
        counts = {}
        lock = threading.Lock()
        ...
        def probe(spec, *args, **kwargs):
            ...
            with lock:
                ...
                final = counts[target] == 2
            if final:
                barrier.wait(timeout=2)
            return ProbeResult("http", target, not (final and "127.0.0.2" in target), 1)

        report = self.execute(transport, probe=probe)
        self.assertEqual(report["tasks"]["host-0"]["status"], "ready")
        self.assertEqual(report["tasks"]["host-1"]["status"], "changed")
        self.assertFalse(report["under_3_minutes"])

    def test_health_probe_cannot_spend_beyond_task_grace(self):
        plan = self.plan()
        task = plan["tasks"][0]
        task["probes"] *= 2
        clock = [100.0]
        ...
        def probe(spec, *args, **kwargs):
            ...
            clock[0] += spec["timeout"]
            return ProbeResult("http", "loopback", True, 1)

        runner = SetupRunner(plan, self.profile, transport=transport, probe=probe)
        runner.started_monotonic = clock[0]
        runner.remaining_at_start = 10
        with patch("sentinel_blue.setup.time.monotonic", side_effect=lambda: clock[0]):
            healthy, checks = runner._healthy(task, plan["hosts"]["host-0"], 0.4)
        self.assertFalse(healthy)
        self.assertEqual(len(limits), 1)
        self.assertAlmostEqual(limits[0], 0.4)
        self.assertEqual(checks[-1]["kind"], "deadline")
    ...
    def test_windows_feature_query_retries_once_inside_original_health_budget(self):
        ...
        runner.started_monotonic, runner.remaining_at_start = 100, 40
        with patch("sentinel_blue.setup.time.monotonic", side_effect=lambda: clock[0]):
    ...
    def test_query_retry_never_replays_runbooks_or_resets_expired_budget(self):
        for recipe, budget, uncertain in (("runbook", 40, True), ("windows-features", 30, True),
                                           ("windows-features", 40, False)):
            with self.subTest(recipe=recipe, budget=budget, uncertain=uncertain):
                plan = self.plan()
                task = dict(plan["tasks"][0], recipe=recipe, probes=[])
                host = dict(plan["hosts"]["host-0"], platform="windows")
                clock, calls = [100.0], []
                def execute(_host, script, seconds):
                    calls.append(script)
                    clock[0] += seconds
                    return CommandResult(1, seconds, uncertain)
                transport = FixtureTransport()
                transport.execute = execute
                runner = SetupRunner(plan, self.profile, transport=transport)
                runner.started_monotonic, runner.remaining_at_start = 100, budget
                with patch("sentinel_blue.setup.time.monotonic", side_effect=lambda: clock[0]):
    ...
    def test_uncertain_initial_readiness_never_starts_setup_mutation(self):
        for recipe, expected_reads in (('runbook', 1), ('windows-features', 2)):
            with self.subTest(recipe=recipe):
                plan = self.plan()
                task = dict(plan['tasks'][0], recipe=recipe, probes=[], timeout_seconds=90)
                plan['hosts']['host-0']['platform'] = 'windows'
                if recipe == 'windows-features':
                    task['check'], task['apply'], _ = compile_recipe(
                        recipe, {'features':['DNS']}, self.root, 'windows')
                calls = []

                def execute(_host, script, _seconds):
                    calls.append(script)
                    return CommandResult(None, 0.1, True)

                transport = FixtureTransport()
                transport.execute = execute
                runner = SetupRunner(plan, self.profile, transport=transport)
                runner.started_monotonic, runner.remaining_at_start = time.monotonic(), 90
                result = runner._task(task)
                self.assertEqual(calls, [task['check']] * expected_reads)
