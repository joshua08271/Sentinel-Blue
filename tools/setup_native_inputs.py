"""Build isolated service fixtures for separately guarded native rehearsals.

Callers must validate their environment before using these privileged inputs.
Windows base services are preserved unless a gated ephemeral runner explicitly
requests a stopped base-service fixture. Only owned sites/zones are removed.
"""
...
def windows_inputs(root: Path, suffix: str, *, stop_base_services: bool = False):
    base = root / "site"
    ...
    # The compiler-owned features task already makes this exact readiness
    # check. A second ServerManager inventory here consumed 120 seconds and
    # failed before setup on the native target. Defer metadata to that real
    # task's initial check; never infer installed features from service names.
    initial = [{"Name": name, "Installed": None,
                "inventory_source": "setup-task-initial-check"}
               for name in ("Web-Server", "DNS")]
    services_before = command("$ErrorActionPreference='Stop'; $ProgressPreference='SilentlyContinue'; $queryErrors=@(); "
        "$rows=@(Get-Service W3SVC,DNS -ErrorAction SilentlyContinue -ErrorVariable queryErrors | "
        "Select-Object Name,@{n='Status';e={[string]$_.Status}},@{n='StartType';e={[string]$_.StartType}}); "
        "if (@($queryErrors | Where-Object {$_.FullyQualifiedErrorId -notlike 'NoServiceFoundForGivenName*'}).Count) "
        "{throw 'Original service inventory was incomplete'}; $rows | ConvertTo-Json -Compress")
    if services_before.returncode or services_before.stderr.strip():
        raise RuntimeError('Original Windows service state was not established')
    before = json.loads(services_before.stdout) if services_before.stdout.strip() else []
    if isinstance(before, dict):
        before = [before]
    if (not isinstance(before, list) or any(
            not isinstance(row, dict) or row.get('Name') not in {'W3SVC', 'DNS'} or
            row.get('Status') not in {'Running', 'Stopped'} or
            row.get('StartType') not in {'Automatic', 'Manual', 'Disabled'} for row in before) or
            len({row['Name'] for row in before}) != len(before)):
        raise RuntimeError('Original Windows service state cannot be restored exactly')
