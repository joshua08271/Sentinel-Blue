...
Version 1.9.26 lets each reachable host begin without waiting for slower host
connections. Preflight and setup share the configured host concurrency limit.
Final service checks, acceptance probes, and resume checks run concurrently
across hosts, with commands on each host still serialized. Package/feature
installation needs one fresh native verification; service stages retain two
consecutive health observations and a separate final verification.
...
`under_30_minutes` is the required timing result; `under_3_minutes` reports the
stretch target separately. Both require complete verified setup. Per-host
preflight, task initial check, apply, health, and final-check timings identify
the observed bottleneck. Neither flag includes VM creation or boot.

These flags also exclude Sentinel Blue upload, deployment, enrollment, and
defense activation. The requested three-minute competition opening includes
those stages and remains unverified. See [competition startup coverage](COMPETITION_STARTUP_COVERAGE.md)
for the service gaps and the complete timing requirement.
...
```bash
python sentinel-blue-1.9.26.pyz setup --inventory private-inventory.json --plan-out private-plan.json
python sentinel-blue-1.9.26.pyz setup --inventory private-inventory.json --execute --approve-plan REVIEWED_SHA256 --state-dir private-setup-state --output setup-result.json
```
