        ...
        if row.get("service_id"):
            ...
            for probe in probes:
                if probe.get("kind") not in {"http", "https", "tcp", "tls", "dns", "transaction", "ftp", "banner", "smtp",
                                             "icmp", "ssh-login", "smb", "mysql", "postgres", "dns-update"}:
...
class SetupRunner:
    def __init__(self, plan: dict, profile: EventProfile, *, transport=None, probe=None, progress=None,
        ...
        self.on_host_ready = on_host_ready
    ...
    def _healthy(self, task: dict, host: dict, seconds: float) -> tuple[bool, list[dict]]:
        ...
        checks = ([{"kind": "native-check", "healthy": True, "attempts": attempts,
                    "retried_read_only_query": True}] if attempts > 1 else [])
        for source in task["probes"]:
            ...
            spec = dict(source)
            spec["timeout"] = min(float(spec.get("timeout", 3)), 3, available)
        ...
        if time.monotonic() >= deadline or self.remaining() <= 0:
            return False, checks + [{"kind": "deadline", "healthy": False}]
        return all(x["healthy"] for x in checks), checks

    def _recheck(self, selected: list[dict]) -> dict[str, tuple[bool, list[dict]]]:
        """Check independent hosts concurrently, with one command per host."""
        groups: dict[str, list[dict]] = {}
        for task in selected:
            groups.setdefault(task["host"], []).append(task)

        def check_host(tasks):
            results = {}
            for task in tasks:
                try:
                    results[task["id"]] = self._healthy(
                        task, self.plan["hosts"][task["host"]], self.remaining())
                except Exception as exc:
                    results[task["id"]] = (False, [{"kind": "check-error", "healthy": False,
    ...
    def _task(self, task: dict) -> dict:
        ...
        initial_check_seconds = time.monotonic() - started
        if healthy:
            return {"status": "ready", "changed": False, "seconds": time.monotonic() - started,
                    "checks": checks, "already_ready": True,
                    "initial_check_seconds": initial_check_seconds}
        if any(check.get('uncertain') is True for check in checks):
            # A timed-out read is not evidence that provisioning is needed.
            # Keep the host held rather than starting a potentially expensive
            # install after the readiness query and its bounded retry failed.
            return {"status": "uncertain", "changed": False,
                    "seconds": time.monotonic() - started,
                    "initial_check_seconds": initial_check_seconds, "checks": checks,
                    "reason": "initial readiness check did not complete; no setup mutation started"}
        ...
        # Installed packages/features are persistent state. One fresh native
        # verification suffices; scored services still require two observations.
        observations = 1 if task["recipe"] in {"linux-packages", "windows-features"} else 2
        consecutive = 0
        while time.monotonic() < grace_end and self.remaining() > 0:
            ...
            if consecutive >= observations:
                return record | {"status": "ready", "seconds": time.monotonic() - started, "checks": checks,
                                 "health_check_seconds": time.monotonic() - health_started}
    ...
    def execute(self, state_dir: Path, approved_digest: str, *, resume: bool = False,
        ...
        if resume and self.on_host_ready is not None:
            raise ValueError("host completion mutations cannot be replayed through setup resume")
        ...
        with _locked_state(state_dir) as state_path:
            ...
            persist()
            host_results = {}
            host_preflight_seconds = {}
            def preflight_host(host):
                started = time.monotonic()
                try:
                    result = self.transport.preflight(host, min(20, self.remaining()))
                    healthy = result.returncode == 0 and not result.uncertain
                except Exception:
                    healthy = False
            ...
            if resume:
                with ThreadPoolExecutor(max_workers=self.plan["max_parallel_hosts"]) as pool:
                    futures = {pool.submit(preflight_host, host): name
                               for name, host in self.plan["hosts"].items()}
                    for future, name in futures.items():
                        host_results[name], host_preflight_seconds[name] = future.result()
                ready = [t for t in self.plan["tasks"] if state["tasks"][t["id"]]["status"] == "ready"]
                for name, (healthy, checks) in self._recheck(ready).items():
                    if not healthy:
                        state["tasks"][name].update(status="changed", checks=checks)
            persist()
            ...
            completions = {}
            completed_hosts = {}
            state["host_completions"] = completed_hosts
            untested_hosts = [] if resume else list(self.plan["hosts"])
            busy = set()
            def complete_host(name):
                started = time.monotonic()
                # Every task already passed native and transaction checks. Final
                # checks still run after deployment; repeating all probes here
                # would add another serialized round to the opening clock.
                if self.remaining() <= 0:
                    return {"status": "deadline"}
                value = self.on_host_ready(name, self.remaining())
                if not isinstance(value, dict) or value.get("status") != "completed":
                    return {"status": "uncertain", "result": value}
                if self.remaining() <= 0:
                    return {"status": "deadline", "result": value}
                return {**value, "seconds": round(time.monotonic() - started, 3)}
            with ThreadPoolExecutor(max_workers=self.plan["max_parallel_hosts"]) as pool:
                while pending or active or preflights or untested_hosts or completions or (
                        self.on_host_ready is not None and len(completed_hosts) < len(self.plan["hosts"])):
                    if self.on_host_ready is not None:
                        for host in self.plan["hosts"]:
                            if host in completed_hosts or host in busy or host not in host_results:
                                continue
                            rows = [state["tasks"][name] for name, task in tasks.items() if task["host"] == host]
                            if (not host_results[host] or host in unsafe_hosts or self.remaining() <= 0 or
                                    any(row["status"] in TERMINAL - {"ready"} for row in rows)):
                                completed_hosts[host] = {"status": "blocked"}
                                persist()
                            elif (all(row["status"] == "ready" for row in rows) and
                                  len(active) + len(preflights) + len(completions) < self.plan["max_parallel_hosts"]):
                                completed_hosts[host] = {"status": "running"}
                                persist()  # durable BEFORE the deployment may mutate
                                busy.add(host)
                                completions[pool.submit(complete_host, host)] = host
                    for name in list(tasks):
                        ...
                        elif host in host_results and not host_results[host]:
                        ...
                        if reason:
                            ...
                            persist()
                            continue
                        if (host not in host_results or host in busy or
                                len(active) + len(preflights) + len(completions) >= self.plan["max_parallel_hosts"] or
                                any(x != "ready" for x in dependency_status)):
                            continue
                        pending.remove(name)
                        busy.add(host)
                    ...
                    # Give an already reachable host its first task immediately;
                    # a slow login on another host never creates a global barrier.
                    while untested_hosts and len(active) + len(preflights) + len(completions) < self.plan["max_parallel_hosts"]:
                        host = untested_hosts.pop(0)
                        if self.remaining() <= 0:
                            host_results[host] = False
                            continue
                        preflights[pool.submit(preflight_host, self.plan["hosts"][host])] = host
                    if active or preflights or completions:
                        finished, _ = wait([*active, *preflights, *completions], timeout=0.25, return_when=FIRST_COMPLETED)
                        for future in finished:
                            if future in completions:
                                host = completions.pop(future)
                                busy.remove(host)
                                try:
                                    completed_hosts[host] = future.result()
                                except Exception as exc:
                                    completed_hosts[host] = {"status": "uncertain", "error": type(exc).__name__}
                                if completed_hosts[host]["status"] != "completed":
                                    unsafe_hosts.add(host)
                                persist()
                                continue
                            if future in preflights:
                                host = preflights.pop(future)
                                host_results[host], host_preflight_seconds[host] = future.result()
                                persist()
                                continue
            ...
            selected = [t for t in self.plan["tasks"]
                        if t["service_key"] and state["tasks"][t["id"]]["status"] == "ready"]
            for name, (healthy, checks) in self._recheck(selected).items():
                row = state["tasks"][name]
                row["final_checks"] = checks
                if not healthy:
                    row["status"] = "deadline" if self.remaining() <= 0 else "changed"
            final_check_seconds = time.monotonic() - final_started
            ...
            completion_ready = self.on_host_ready is None or (
                len(completed_hosts) == len(self.plan["hosts"]) and
                all(row["status"] == "completed" for row in completed_hosts.values()))
            complete = all_ready and completion_ready and not self.plan["uncovered_services"] and elapsed < self.plan["budget_seconds"]
            report = {
                ...
                "under_3_minutes": complete and elapsed < 180,
                ...
                "host_completions": completed_hosts,
                "host_preflight_seconds": host_preflight_seconds,
                "final_check_seconds": round(final_check_seconds, 3),
