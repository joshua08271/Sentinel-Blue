...
class _WindowsNativeFileOps:
    ...
    def read_file(self, handle, maximum: int, *, read_budget=None) -> bytes:
        ...
        while remaining:
            ...
            if not 0 <= count <= chunk_size:
                raise OSError("Windows restoration read returned an invalid byte count")
            if read_budget is not None:
                # Charge actual reads even if later metadata validation fails.
                read_budget.consume(count)
            if count == 0:
                break
...
def _windows_snapshot_open_handle(
    ...
    *,
    ...
    if int(before["links"]) != 1 and not (allow_hard_links and int(before['links']) > 0):
    ...
    data = (
        native.read_file(handle, maximum, read_budget=read_budget)
        if read_budget is not None else native.read_file(handle, maximum)
    )
    if len(data) > maximum:
        raise ValueError(
            f"Windows restoration file exceeds its {maximum} byte limit"
...
class RestorePointStore:
    ...
    def restore(
        ...
        *,
        ...
        # A composite service repair reserves the child identity durably before
        # the first write, so a crash cannot orphan a successful file mutation.
        reserved = transaction_id is not None
        transaction_id = transaction_id or str(uuid.uuid4())
        if not TRANSACTION_ID.fullmatch(transaction_id):
            raise ValueError("invalid reserved restoration transaction identifier")
        if reserved and self._read_private_file_if_present(self.transactions / f"{transaction_id}.json", MAX_TRANSACTION_BYTES) is not None:
            raise ValueError("reserved restoration transaction already exists")
        ...
        try:
            ...
            validation = ({"applicable": False, "available": False, "healthy": None,
                           "detail": "validation belongs to the parent service repair"}
                          if defer_validation else validate_restored_configuration(path))
            if (
    ...
    @classmethod
    def _read_target(cls, path: Path, *, maximum: int = MAX_FILE_BYTES) -> tuple[bytes, dict[str, Any]]:
        # Inventory may hash executables larger than a restorable configuration.
        # All restoration/quarantine callers retain the smaller default limit.
        if isinstance(maximum, bool) or not isinstance(maximum, int) or not 0 < maximum <= 32*1024*1024:
            raise ValueError('invalid bounded target read size')
        cls._validate_windows_path(path)
        if os.name == "nt":
            data, snapshot = _windows_read_file_snapshot(path, maximum)
        ...
        try:
            ...
            if not stat.S_ISREG(before_info.st_mode):
                raise ValueError("target is not a regular non-symlink file")
            if before_info.st_size > maximum:
                raise ValueError(f"target exceeds {maximum} byte bounded read limit")
