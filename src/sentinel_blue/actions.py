...
class ActionExecutor:
    ...
    def _service_recovery_preflight(self, parameters: dict[str, Any]) -> None:
        guard = parameters.get("recovery_guard")
        if guard is None:
            return
        now = time.time()
        records = list(self.state_dir.glob("service-transaction-*.json"))
        if len(records) > 1024:
            raise ValueError("service recovery history exceeds its inspection budget")
        attempts = []
        for path in records:
            item = read_private_json(path, 128 * 1024)
            ...
            if item.get("operation") == "restart_service" and now - created < RECOVERY_WINDOW_SECONDS:
